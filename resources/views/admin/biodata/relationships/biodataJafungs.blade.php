                ...
                <tbody>
                    @foreach($jafungs as $key => $jafung)
                        <tr data-entry-id="{{ $jafung->id }}">
                            ...
                            <td>
                                @can('jafung_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.jafungs.show', $jafung->id) }}">
                                        View
                                    </a>
                                @endcan

                                @can('jafung_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.jafungs.edit', $jafung->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('jafung_delete')
                                    <form action="{{ route('admin.jafungs.destroy', $jafung->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
