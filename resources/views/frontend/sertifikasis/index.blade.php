...
@can('sertifikasi_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('frontend.sertifikasis.create') }}">
                        ...
                        <tr data-entry-id="{{ $sertifikasi->id }}">
                            ...
                            <td>
                                @can('sertifikasi_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('frontend.sertifikasis.show', $sertifikasi->id) }}">
                                        View
                                    </a>
                                @endcan

                                @can('sertifikasi_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('frontend.sertifikasis.edit', $sertifikasi->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('sertifikasi_delete')
                                    <form action="{{ route('frontend.sertifikasis.destroy', $sertifikasi->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
