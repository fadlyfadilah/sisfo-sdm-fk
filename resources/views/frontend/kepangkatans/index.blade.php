...
@can('kepangkatan_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('frontend.kepangkatans.create') }}">
                Tambah Kepangkatan
            </a>
        </div>
    </div>
                        ...
                        <tr data-entry-id="{{ $kepangkatan->id }}">
                            ...
                            <td>
                                @can('kepangkatan_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('frontend.kepangkatans.show', $kepangkatan->id) }}">
                                        View
                                    </a>
                                @endcan

                                @can('kepangkatan_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('frontend.kepangkatans.edit', $kepangkatan->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('kepangkatan_delete')
                                    <form action="{{ route('frontend.kepangkatans.destroy', $kepangkatan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
