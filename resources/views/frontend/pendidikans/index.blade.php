...
@can('pendidikan_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('frontend.pendidikans.create') }}">
                ...
                <tbody>
                    @foreach($pendidikans as $key => $pendidikan)
                        <tr data-entry-id="{{ $pendidikan->id }}">
                            ...
                            <td>
                                @can('pendidikan_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('frontend.pendidikans.show', $pendidikan->id) }}">
                                       View
                                    </a>
                                @endcan

                                @can('pendidikan_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('frontend.pendidikans.edit', $pendidikan->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('pendidikan_delete')
                                    <form action="{{ route('frontend.pendidikans.destroy', $pendidikan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
